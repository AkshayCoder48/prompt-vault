import { NextRequest, NextResponse } from "next/server";
import { analyticsService } from "@/lib/onyxbase/analytics";
import { verifyAdmin } from "@/lib/admin";
import { OnyxBaseError } from "@/lib/onyxbase/client";

export const dynamic = "force-dynamic";

export async function GET(req: NextRequest) {
  if (!(await verifyAdmin(req))) {
    return NextResponse.json({ ok: false, error: "Unauthorized." }, { status: 401 });
  }
  try {
    const [dashboard, topSearches] = await Promise.all([
      analyticsService.getDashboard(),
      analyticsService.getTopSearchTerms(10),
    ]);
    return NextResponse.json({ ok: true, ...dashboard, topSearches });
  } catch (err: any) {
    const status = err instanceof OnyxBaseError ? err.status || 503 : 500;
    return NextResponse.json(
      { ok: false, error: err.message || "Failed to load analytics.", retryable: status >= 500 },
      { status: status >= 500 ? 503 : 500 }
    );
  }
}
